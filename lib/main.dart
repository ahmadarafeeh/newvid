...
void main() async {
  ...
  // 🚀 WEB-SPECIFIC FIX: Completely different approach for web vs mobile
  if (kIsWeb) {
  ...
  // 🚀 CRITICAL: Start app IMMEDIATELY
  runApp(
...
// 🛡️ WEB: Minimal initialization to avoid JavaScript errors
Future<void> _initializeForWeb() async {
  try {
    // 1. Only initialize Firebase if absolutely necessary for web
    // Many web apps work without immediate Firebase init
    ...
    // 2. Supabase only - no other services
    ...
    // 🛡️ CRITICAL: On web, we MUST continue even if services fail
    debugPrint('Web initialization error (continuing anyway): $e');
  }
}

// 🚀 MOBILE: Full initialization for native platforms
Future<void> _initializeForMobile() async {
  try {
    // 1. Firebase for mobile
    await Firebase.initializeApp();
