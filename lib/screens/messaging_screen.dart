...
class _MessagingScreenState extends State<MessagingScreen>
  ...
  // Debug counter
  int _messageBuildCount = 0;
  int _bubbleBuildCount = 0;
  ...
  Future<void> _loadInitialMessages() async {
    ...
    try {
      ...
      if (initialMessages.isNotEmpty) {
        final oldest = _parseTimestamp(initialMessages.last['timestamp']);
        if (oldest != null) {
          _oldestMessageTimestamp = oldest;
        }
      }
      ...
      setState(() {
        _cachedMessages = initialMessages;
        _currentPage = 0;
  ...
  void _handleSwipeUpdate(String messageId, DragUpdateDetails details) {
    ...
    double newOffset = (_swipeOffsets[messageId] ?? 0) + details.delta.dx;
  ...
  Future<void> _sendMessage() async {
    ...
    final optimisticMessage = {
      ...
      'repliedMessagePreview': _replyingToMessage != null
          ? _getMessagePreview(_replyingToMessage!)
          : null,
      'repliedMessageSender': _replyingToMessage != null
          ? (_replyingToMessage!['senderId'] == currentUserId ? 'You' : 'Them')
          : null,
      'repliedMessageType': _replyingToMessage?['type'] ?? 'text',
    };
  ...
  Future<void> _refreshMessages() async {
    ...
    try {
      ...
      if (refreshedMessages.isNotEmpty) {
        final oldest = _parseTimestamp(refreshedMessages.last['timestamp']);
        if (oldest != null) {
          _oldestMessageTimestamp = oldest;
        }
      }
      ...
      setState(() {
        _cachedMessages = refreshedMessages;
        _hasMoreMessages =
            refreshedMessages.length >= ((_currentPage + 1) * _messagesPerPage);
      });
    } catch (e) {}
  }
  ...
  Widget _buildMessageList(_MessagingColorSet colors) {
    ...
    return Column(
      children: [
        ...
        // Messages list - removed the Load older messages button
        Expanded(
  ...
  Widget _buildReplyIndicator(
    ...
    final repliedMessagePreview = message['repliedMessagePreview'] ?? '';
    final repliedMessageSender = message['repliedMessageSender'] ?? '';
