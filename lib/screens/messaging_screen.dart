...
// Import the falling numbers files
...
class _MessagingColorSet {
  ...
  final Color currentUserMessageColor; // Color for current user's messages
  final Color otherUserMessageColor; // Color for other user's messages
  ...
  _MessagingDarkColors()
      : super(
          ...
          currentUserMessageColor:
              const Color(0xFF333333), // Dark gray for current user
          otherUserMessageColor:
              const Color(0xFF404040), // Slightly lighter for other user
...
class _MessagingLightColors extends _MessagingColorSet {
  _MessagingLightColors()
      : super(
          ...
          currentUserMessageColor:
              Color(0xFFF0F0F0), // Very light gray for current user
          otherUserMessageColor:
              Color(0xFFE0E0E0), // Slightly darker gray for other user
...
class _MessagingScreenState extends State<MessagingScreen>
  ...
  // Add optimistic messages list
  final List<Map<String, dynamic>> _optimisticMessages = [];

  // Falling numbers animation variables
  ...
  void _setupKeyboardListener() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (viewInsets.bottom > 0) {
        // Keyboard is already visible, ensure we're scrolled to bottom
        _scrollToBottom();
      }
    });
  ...
  void _sendMessage() async {
    if (_controller.text.isEmpty || _isMutuallyBlocked) return;

    // Capture the message text before clearing
    final messageText = _controller.text.trim();

    // OPTIMISTIC UPDATE: Create and add optimistic message immediately
    ...
    try {
      ...
      // The optimistic message will be automatically replaced when the stream updates

      // Scroll again after message is sent to ensure visibility
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _scrollToBottom();
      });
    } catch (e) {
  ...
  // Helper method to check if timestamps are close enough to be considered the same message
  bool _areTimestampsClose(DateTime? timestamp1, DateTime? timestamp2) {
    ...
    // Consider messages within 30 seconds as the same
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
            // Falling numbers background - only show if not blocked
            if (!_isMutuallyBlocked)
  ...
  Widget _buildMessageList(_MessagingColorSet colors) {
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        List<Map<String, dynamic>> serverMessages = snapshot.data ?? [];

        // COMBINE: Merge server messages with optimistic messages
        List<Map<String, dynamic>> allMessages = [...serverMessages];

        // Add optimistic messages that aren't yet confirmed by the server
        for (final optimisticMsg in _optimisticMessages) {
          // Check if this optimistic message has been confirmed by the server
          final isConfirmed = serverMessages.any((serverMsg) {
        ...
        if (serverMessages.isNotEmpty && _optimisticMessages.isNotEmpty) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted) {
              setState(() {
                _optimisticMessages.removeWhere((optimisticMsg) {
                  return serverMessages.any((serverMsg) {
                    return serverMsg['message'] == optimisticMsg['message'] &&
                        serverMsg['senderId'] == optimisticMsg['senderId'] &&
                        _areTimestampsClose(
                            _parseTimestamp(serverMsg['timestamp']),
                            optimisticMsg['timestamp']);
                  });
                });
              });
        ...
        // When we get new data and keyboard is open, scroll to bottom
        if (snapshot.hasData &&
        ...
        // Only show empty state if we have data from the stream and there are no messages
        final shouldShowEmptyState = snapshot.hasData &&
        ...
        // If we're still waiting for data but have optimistic messages, show them
        if (allMessages.isEmpty && _optimisticMessages.isNotEmpty) {
...
// Separate widget for falling numbers to isolate the animation
