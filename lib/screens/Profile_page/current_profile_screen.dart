...
class _CurrentUserProfileScreenState extends State<CurrentUserProfileScreen> {
  ...
  // ========== UPDATED: Gallery video player that fills entire space ==========
  ...
  Future<void> getData() async {
    ...
    try {
      // First, get total post count using a simple query
      ...
      final List<Future<dynamic>> queries = [
        ...
        // We already have initial posts, pass them along
        ...
        _supabase
            ...
            .eq('user_id', widget.uid),
        // Fetch galleries with post counts and cover images
      ...
      if (mounted) {
        setState(() {
          ...
          postCount = totalPostCount; // Set total post count
  ...
  // Helper method to build username with flag using the reusable CountryFlagWidget
  Widget _buildUsernameWithFlag(
    ...
    return Row(
      ...
      children: [
        ...
        // Country flag using the reusable widget - BETWEEN USERNAME AND VERIFICATION
        if (hasCountryFlag) ...[
  ...
  Widget _buildPostsGrid(_ColorSet colors) {
    return GridView.builder(
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
      itemCount: _displayedPosts.length + 1, // +1 for add button
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 3,
  ...
  // Galleries Grid - UPDATED with video that fills entire space
  Widget _buildGalleriesGrid(_ColorSet colors) {
    ...
    return GridView.builder(
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
      itemCount: _galleries.length + 1, // +1 for add button
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 2,
        crossAxisSpacing: 8,
        mainAxisSpacing: 8,
  ...
  Widget _buildGalleryItem(Map<String, dynamic> gallery, _ColorSet colors) {
    ...
    return GestureDetector(
      onTap: () {
        // Navigate to gallery detail screen
        Navigator.push(
          context,
          MaterialPageRoute(
          ...
          // Refresh gallery data when returning from gallery detail
          getData();
        });
      },
      child: Container(
  ...
  void _createNewGallery() {
    // Get the theme provider and colors within the method
