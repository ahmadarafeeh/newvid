    ...
    if (_trimDirty) {
      ...
      await _log(operation: 'trim/start', data: {
        ...
        'originalSizeBytes': widget.videoFile.existsSync()
            ? widget.videoFile.lengthSync()
            : null,
      });

      try {
        // saveTrimmedVideo fires onSave synchronously inside its own async
        // work, but some versions fire it after the future resolves.
        // We wrap in a Completer so we truly await the callback.
        ...
        // Give the callback up to 30 s to fire in case it is deferred.
        ...
        if (savedPath != null) {
          ...
          if (!exists || sizeBytes == 0) {
            await _log(
              ...
              errorMessage: exists
                  ...
                  : 'File does not exist at saved path',
              data: {
                'savedPath': savedPath,
                'exists': exists,
                'sizeBytes': sizeBytes
          ...
          await _log(
            ...
            errorMessage: 'onSave callback returned null path or timed out',
            data: {'startValue': _startValue, 'endValue': _endValue},
          );
        }
      } catch (e, st) {
        trimError = e.toString();
        trimStack = st.toString();
        await _log(
          operation: 'trim/exception',
      ...
      await _log(operation: 'trim/navigate', data: {
        ...
        'finalSizeBytes':
            fileToPass.existsSync() ? fileToPass.lengthSync() : null,
        'trimError': trimError,
      });
