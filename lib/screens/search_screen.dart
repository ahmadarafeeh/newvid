...
class _SearchScreenState extends State<SearchScreen> {
  ...
  // Video player controllers cache - EXACT SAME AS CurrentUserProfileScreen
  ...
  void dispose() {
    ...
    // Dispose all video controllers - EXACT SAME AS CurrentUserProfileScreen
  ...
  // -------------------------
  // Video player logic - EXACT SAME AS CurrentUserProfileScreen
  // -------------------------
  ...
  /// Configure video to play only first second on loop - EXACT SAME AS CurrentUserProfileScreen
  void _configureVideoLoop(VideoPlayerController controller) {
    ...
    // Start playing - EXACT SAME AS CurrentUserProfileScreen
    controller.play();
  }
  ...
  Future<void> _fetchPosts() async {
    try {
      ...
      if (response is List && response.isNotEmpty) {
        ...
        // Initialize video controllers for all video posts - EXACT SAME AS CurrentUserProfileScreen
  ...
  Future<void> _loadMorePosts() async {
    ...
    try {
      ...
      if (response is List && response.isNotEmpty) {
        ...
        // Initialize video controllers for new video posts - EXACT SAME AS CurrentUserProfileScreen
    ...
    return ListView(
      ...
      children: [
        ...
        GridView.builder(
          ...
          itemCount: _initialPostsToShow - 3, // Adjusted based on initial posts
          itemBuilder: (context, index) => _buildPostSkeleton(colors),
        ),
      ],
    );
  ...
  Widget _buildPostItem(Map<String, dynamic> post, String postUrl,
    ...
    // Start initialization if it's a video - EXACT SAME AS CurrentUserProfileScreen
    if (isVideo) {
      _initializeVideoController(postUrl);
    }
