...
// Helper to log success events (non‑error)
Future<void> _logSuccessEvent({
  required String eventType,
  String? firebaseUid,
  String? supabaseUid,
  String? email,
...
class _AuthWrapperState extends State<AuthWrapper> {
  ...
  Future<void> _handleSupabaseSession(
    ...
    try {
      ...
      // --- STEP 3: Try to find by supabase_uid ---
      if (!found) {
      ...
      // Log successful session handling
      await _logSuccessEvent(
      ...
      await _logErrorEvent(
        ...
        errorDetails: e.toString(),
        stackTrace: stack.toString(),
        additionalData: {'recordSource': recordSource},
      );
  ...
  Widget build(BuildContext context) {
    ...
    final bool hasUser = isFirebaseUser || isSupabaseUser;

    // Determine target screen and log decision
    String targetScreen;
    ...
    // Log the navigation decision (non-error)
