...
// Helper to log only error events to login_logs table
...
class _AuthWrapperState extends State<AuthWrapper> {
    ...
    try {
      ...
      if (firebaseUser != null) {
        ...
        if (userData != null) {
          ...
          // Delete any duplicate record that may have been created with this supabase_uid
          await _supabase
              .from('users')
              .delete()
      ...
      if (!found && session.user.email != null) {
        ...
        if (userByEmail != null) {
          ...
          // Delete any duplicate record with this supabase_uid
          await _supabase
              .from('users')
              .delete()
              .eq('supabase_uid', session.user.id)
      ...
      if (!found) {
        ...
        if (records.isNotEmpty) {
          ...
          // If there's more than one, pick one and delete extras.
          if (records.length > 1) {
  ...
  // --- The remaining methods are unchanged except for removal of non‑error logs ---
