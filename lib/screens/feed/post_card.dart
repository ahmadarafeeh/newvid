...
class VideoManager {
  ...
  // 🎯 ADDED: Static method to pause all videos globally
...
class _PostCardState extends State<PostCard>
  ...
  // 🎯 CHANGED: Initialize showPlayButton to false to prevent it from appearing at start
  ...
  // Get video playing state from VideoManager - USING ONLY THIS GETTER
  ...
  void dispose() {
    ...
    // 🎯 ADDED: Ensure video is removed from VideoManager when disposed
  ...
  // 🎯 UPDATED: Video listener with modified play button state management
  void _videoListener() {
    if (!mounted) return;
    ...
    if (wasPlaying != isNowPlaying) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        setState(() {
          // 🎯 CHANGED: Only show play button when video is paused AND was previously playing
          // This prevents the play button from appearing at the very start
          _showPlayButton = !isNowPlaying && wasPlaying;
        });
      });
    }
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _isVideoLoading = false;
          // 🎯 CHANGED: Don't show play button at start
  ...
  void _playVideo() {
    if (_videoController != null &&
      ...
      // 🎯 CHANGED: Keep play button hidden when video starts playing
      setState(() {
        _showPlayButton = false;
      });
  ...
  void _pauseVideo() {
    if (_videoController != null && _isVideoInitialized && mounted) {
      ...
      // 🎯 CHANGED: Show play button when video is manually paused
      setState(() {
  ...
  // 🎯 UPDATED: Toggle video playback with modified play button logic
  ...
  // Rating submission handler - updated for immediate optimistic updates
  void _handleRatingSubmitted(double rating) async {
    final user = Provider.of<UserProvider>(context, listen: false).user;
    if (user == null) return;

    // CAPTURE THE OLD RATING BEFORE UPDATING STATE
    ...
    setState(() {
      ...
      // CORRECT optimistic update logic:
      ...
      // Update local ratings with optimistic data
    ...
    // Make API call in background - no loading state
    try {
      ...
      if (mounted) {
        // If error occurred, refetch to restore correct state
        _fetchInitialRatings();
      }
    }
  }
...
// Fixed caption with proper gesture handling
  Widget _buildCaptionWithVisibility(_ColorSet colors) {
    final caption = widget.snap['description'].toString();

    // Check if text is long enough to require truncation
    final bool needsTruncation =
        caption.length > 80; // Adjust this threshold as needed
  ...
  // Comment button with original styling
  ...
  // Right action buttons with original styling - MUTE BUTTON BACK IN ORIGINAL POSITION
  Widget _buildRightActionButtons(_ColorSet colors) {
    return Column(
      children: [
        ...
        const SizedBox(height: 8),
        // 🎯 MUTE BUTTON BACK IN ORIGINAL POSITION
  ...
  // Bottom overlay with TikTok-style text visibility - TIMESTAMP REMOVED
        ...
        children: [
          ...
          Row(
            children: [
              ...
              // Rating summary remains in the same position
              Container(
                decoration: BoxDecoration(
          ...
          const SizedBox(height: 8),
          // Caption with TikTok-style visibility
  ...
  // 🎯 UPDATED: Video player with improved play/pause behavior
        ...
        children: [
          ...
          if (_showPlayButton && _isVideoInitialized)
            AnimatedOpacity(
              ...
              child: Center(
                child: GestureDetector(
                  onTap: _playVideo, // 🎯 Tap play button to play
                  child: Container(
                    width: 80,
                    height: 80,
                    decoration: BoxDecoration(
          ...
          // 🎯 REMOVED: Mute button from video overlay (now in right actions)
        ],
      ),
    );
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            bottom: 260, // 🎯 MOVED UP: from 220 to 260
            right: 16,
