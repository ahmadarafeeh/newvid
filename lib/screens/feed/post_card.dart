...
class _PostCardState extends State<PostCard>
  ...
  // 🎯 CHANGED: Use getter from VideoManager instead of local state variable
    ...
    if (oldWidget.isVisible != widget.isVisible && _isVideo) {
      if (widget.isVisible) {
        if (_isVideoInitialized && !_isVideoPlaying) {
          ...
          _initializeVideoPlayer(); // 🎯 CHANGED: Added this condition
        }
      } else {
        if (_isVideoInitialized && _isVideoPlaying) {
          _pauseVideo();
  ...
  // 🎯 CHANGED: Simplified video listener - only handles video looping
  void _videoListener() {
    if (!mounted) return;
  ...
  // 🎯 CHANGED: Toggle video playback with simplified logic
  ...
  // 🎯 CHANGED: Video player with fixed play button behavior from Code B
    ...
    return Container(
      ...
      child: Stack(
        ...
        children: [
          ...
          // 🎯 CHANGED: Only show play button when video is NOT playing and initialized
