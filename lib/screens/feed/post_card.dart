...
class VideoManager {
  ...
  // Track all active video controllers
  ...
  // New method to pause ALL active videos
...
class _PostCardState extends State<PostCard>
  ...
  bool get _isProfileVideo {
    final url = (widget.snap['profImage']?.toString() ?? '').toLowerCase();
  ...
  void initState() {
    ...
    if (_isVideo) {
      // 🎯 CRITICAL FIX: Initialize video immediately for first post with small delay
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      // Pause both videos when app goes to background
      _pauseVideo();
      _pauseProfileVideo();
    }
  }
  ...
  void didUpdateWidget(PostCard oldWidget) {
    ...
    // Improved video visibility logic with better first post handling
    ...
    // Handle profile video visibility
    if (oldWidget.isVisible != widget.isVisible && _isProfileVideo) {
      if (widget.isVisible) {
        // Only play if profile video is initialized
        ...
        // Pause profile video when not visible
        if (_isProfileVideoInitialized) {
          _pauseProfileVideo();
        }
      }
  ...
  // Improved video listener with better state management
  void _videoListener() {
    if (!mounted) return;
    ...
    // Sync playing state with VideoManager
  ...
  // ADDED: Profile video listener
  void _profileVideoListener() {
    if (!mounted) return;

    // Handle profile video looping
    if (_profileVideoController != null &&
  ...
  Future<void> _initializeProfileVideo() async {
    ...
    try {
      final videoUrl = widget.snap['profImage']?.toString() ?? '';
      if (videoUrl.isEmpty) {
  ...
  // Improved video toggle with proper state sync
  ...
  Widget _buildProfilePicture(_ColorSet colors) {
    ...
    if (isVideo) {
      return _buildProfileVideoPlayer();
    }
  ...
  // Video player with reliable play button
  ...
  void _navigateToProfile() {
    // Ensure both videos are properly paused before navigation
    if (_isVideo && _isVideoInitialized) {
      _pauseVideo();
    }
  ...
  void _openCommentsPanel() {
    // Pause both videos before opening comments
    if (_isVideo && _isVideoInitialized) {
      _pauseVideo();
    }
    if (_isProfileVideo && _isProfileVideoInitialized) {
  ...
  void _navigateToShare(_ColorSet colors) {
    // Ensure both videos are properly paused before showing share dialog
    if (_isVideo && _isVideoInitialized) {
      _pauseVideo();
    }
    if (_isProfileVideo && _isProfileVideoInitialized) {
