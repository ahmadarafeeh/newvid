...
class _FeedScreenState extends State<FeedScreen> {
  ...
  // NEW: Flag to track if first video has been initialized
  ...
  // Load cached posts immediately for instant UI
      ...
      try {
        ...
        if (cachedPosts != null && cachedPosts.isNotEmpty && mounted) {
          setState(() {
            ...
            // 🎯 CRITICAL FIX: Set initial visibility for first post
            if (_forYouPosts.isNotEmpty) {
              ...
              if (firstPostId.isNotEmpty) {
                ...
                _scheduleViewRecording(firstPostId);
                _firstVideoInitialized = false; // Reset flag
              }
            }
          });
        } else {
  ...
  void initState() {
    ...
    // Show UI immediately with cached posts
  ...
  void _updatePostVisibility(
    ...
    setState(() {
      ...
      if (page < posts.length) {
        ...
        if (postId.isNotEmpty) {
          ...
          // 🎯 CRITICAL FIX: Special handling for first post
    ...
    if (newPlayingPostId != null &&
      ...
      // Only pause previous video if we have a new one to play
    ...
    try {
      ...
      if (_essentialUiReady && _forYouPosts.isNotEmpty) {
        ...
        if (mounted) {
          ...
          // 🎯 CRITICAL FIX: Immediately set first post as visible and playing
            ...
            if (firstPostId.isNotEmpty) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                if (mounted) {
                  ...
                  _scheduleViewRecording(firstPostId);
                  _preloadNextPosts(_forYouPosts, 0, true);
                }
              });
            }
          }
      ...
      if (mounted) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted && !loadMore) {
            ...
            // 🎯 CRITICAL FIX: Set initial visibility for first post after fresh data load
  ...
  void _switchTab(int index) {
    ...
    _pauseCurrentVideo();
    _currentPlayingPostId = null;
    _firstVideoInitialized = false; // Reset flag on tab switch
  ...
  bool _shouldPostPlayVideo(String postId) {
    // Only play video if it's the current post AND visible
    return postId == _currentPlayingPostId && (_postVisibility[postId] == true);
  }

  // Build skeleton UI for immediate display
  Widget _buildMinimalSkeleton(_ColorSet colors) {
    return PageView.builder(
      scrollDirection: Axis.vertical,
      itemCount: 1, // Single skeleton item for faster rendering
  ...
  void _navigateToMessages() {
    // Properly pause all videos before navigation
    VideoManager.pauseAllVideos();
    _currentPlayingPostId = null;
    ...
    // Ensure video is truly paused before navigation
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Navigator.push(
        context,
        MaterialPageRoute(
