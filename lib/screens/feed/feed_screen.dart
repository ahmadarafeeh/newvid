...
class _FeedScreenState extends State<FeedScreen> {
  ...
  // NEW: Variables for tab visibility control
  ...
  // NEW: Methods for tab visibility control
  void _showTabs() {
    if (_hideTabsTimer != null) {
  ...
  void dispose() {
    ...
    _hideTabsTimer?.cancel(); // Clean up the timer
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        onTap: () {
          // Show tabs when user taps anywhere on the screen
          _showTabs();
        },
        child: Stack(
          children: [
            ...
            // NEW: Use conditional rendering based on _tabsVisible
            if (width <= webScreenSize) _buildOverlayTabs(colors),
            // NEW: Always show message button when tabs are visible
  ...
  // NEW: Split the static overlay into separate methods
  Widget _buildOverlayTabs(_ColorSet colors) {
    return AnimatedPositioned(
      duration: const Duration(milliseconds: 300),
      curve: Curves.easeInOut,
      top: _tabsVisible ? MediaQuery.of(context).padding.top + 8 : -100,
      left: 0,
      right: 0,
      child: Container(
