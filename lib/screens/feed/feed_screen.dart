...
class _FeedScreenState extends State<FeedScreen> {
  ...
  double _previousForYouPage = 0.0;
  double _previousFollowingPage = 0.0;
  ...
  void initState() {
    ...
    _previousForYouPage = 0;
    _previousFollowingPage = 0;
  ...
  void _addScrollListeners() {
    _forYouPageController.addListener(_handleForYouScroll);
    _followingPageController.addListener(_handleFollowingScroll);
  }

  void _handleForYouScroll() {
    if (!_forYouPageController.position.hasContentDimensions) return;

    final currentPage = _forYouPageController.page ?? 0;
    _handlePageViewScroll(currentPage, true);
  }

  void _handleFollowingScroll() {
    if (!_followingPageController.position.hasContentDimensions) return;

    final currentPage = _followingPageController.page ?? 0;
    _handlePageViewScroll(currentPage, false);
  }

  void _handlePageViewScroll(double currentPage, bool isForYou) {
    if (!mounted) return;
    ...
    if (isForYou) {
      previousPage = _previousForYouPage;
      scrollDelta = currentPage - previousPage;
      _previousForYouPage = currentPage;
    } else {
      previousPage = _previousFollowingPage;
      scrollDelta = currentPage - previousPage;
      _previousFollowingPage = currentPage;
    }

    if (scrollDelta > 0.1 && _showOverlay) {
      setState(() {
        _showOverlay = false;
      });
    } else if (scrollDelta < -0.1 && !_showOverlay) {
      setState(() {
        _showOverlay = true;
      });
    }
  ...
  void dispose() {
    _pauseCurrentVideo();
    _currentPlayingPostId = null;
    _followingPageController.removeListener(_handleFollowingScroll);
    _forYouPageController.removeListener(_handleForYouScroll);
