...
class _FeedScreenState extends State<FeedScreen> {
  ...
  // NEW: Scroll hide/show variables
  ...
  void initState() {
    ...
    // Add scroll listeners after controllers are initialized
  ...
  // NEW: Add scroll listeners for hide/show behavior
  ...
  void _handleForYouScroll() {
    ...
    final currentOffset = _forYouPageController.offset;
    _handleScroll(currentOffset);
  }

  void _handleFollowingScroll() {
    if (!_followingPageController.position.hasContentDimensions) return;
    final currentOffset = _followingPageController.offset;
    _handleScroll(currentOffset);
  }

  void _handleScroll(double currentOffset) {
    if (!mounted) return;

    final scrollDelta = currentOffset - _lastScrollOffset;

    // Hide overlay when scrolling down, show when scrolling up
    if (scrollDelta > _scrollThreshold && _showOverlay) {
      // Scrolling down - hide overlay
      setState(() {
        _showOverlay = false;
      });
      _lastScrollOffset = currentOffset;
    } else if (scrollDelta < -_scrollThreshold && !_showOverlay) {
      // Scrolling up - show overlay
      setState(() {
        _showOverlay = true;
      });
      _lastScrollOffset = currentOffset;
