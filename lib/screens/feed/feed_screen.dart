    ...
    try {
      ...
      if (cachedPosts != null && cachedPosts.isNotEmpty && mounted) {
        ...
        if (unseenCachedPosts.length >= 2) {
          ...
          debugPrint(
              '🎯 FeedScreen: Showing ${unseenCachedPosts.length} unseen cached posts (from ${cachedPosts.length} total cached)');
          ...
          if (unseenCachedPosts.length == 1) {
            debugPrint(
                '📝 FeedScreen: Skipping cache - only 1 unseen post available');
          } else {
            debugPrint('📝 FeedScreen: No usable cached posts available');
          }
        }
      } else {
        setState(() {
      ...
      debugPrint('❌ FeedScreen: Cache load error: $e');
    }
  }
