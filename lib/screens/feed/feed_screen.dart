...
class _FeedScreenState extends State<FeedScreen> with WidgetsBindingObserver {
  ...
  Future<Set<String>> _getSeenPosts() async {
    try {
      ...
      final seenPostsList =
          prefs.getStringList('seen_posts$currentUserId') ?? [];
  ...
  Future<void> _markPostAsSeen(String postId) async {
    try {
      ...
      await prefs.setStringList('seen_posts$currentUserId', trimmedSeenPosts);
    } catch (e) {}
  }
  ...
  void _initializeHeavyComponents() {
    ...
    // MOVED TO BACKGROUND: Load ads after feed is visible
    _scheduleBackgroundAdLoading();
  }

  // NEW: Schedule ad loading for after initial render
  void _scheduleBackgroundAdLoading() {
    // Wait until after first build and a short delay
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  Future<void> _loadEssentialDataInBackground() async {
    ...
    try {
      // FIRE-AND-FORGET: Don't await blocked users for initial UI
      unawaited(_loadBlockedUsers().then((_) {
        _essentialDataLoaded = true;
        _scheduleRebuild();

        // Load data after blocked users are loaded
  ...
  void _updatePostVisibility(
    ...
    // Schedule single rebuild instead of immediate setState
    _scheduleRebuild();
  ...
  // NEW: Update visibility without triggering rebuild
  ...
  void _onPageChanged(int page, bool isForYou) {
    ...
    if (isForYou) {
      ...
      // LOAD ADS WHEN USER STARTS SCROLLING (first interaction)
  ...
  // UPDATED: Background ad loading that doesn't block feed
        ...
        onAdLoaded: (InterstitialAd ad) {
          ...
          _interstitialAd!.fullScreenContentCallback =
            ...
            onAdDismissedFullScreenContent: (InterstitialAd ad) {
              ad.dispose();
              // RELOAD IN BACKGROUND AFTER DISMISSAL
              _scheduleBackgroundAdLoading();
            },
            onAdFailedToShowFullScreenContent:
                (InterstitialAd ad, AdError error) {
              ad.dispose();
              // RELOAD IN BACKGROUND AFTER FAILURE
              _scheduleBackgroundAdLoading();
            },
          );
        },
  ...
  // ULTRA-FAST: Updated _loadBlockedUsers with preloading support
  Future<void> _loadBlockedUsers() async {
    // ULTRA-FAST: Check preloaded data first
  ...
  // ULTRA-FAST: Updated _loadFreshDataInBackground with faster timeouts
  Future<void> _loadFreshDataInBackground() async {
    // DON'T EVEN TRY if we don't have blocked users yet
    ...
    try {
      ...
      // ADD FOLLOWING IDS TO EXCLUDED USERS FOR FOR YOU TAB
      final excludedUsers = [..._blockedUsers, currentUserId, ..._followingIds];

      // ULTRA-FAST: Use shorter timeout and request fewer posts
      final responseRaw = await _supabase.rpc('get_for_you_feed', params: {
        'current_user_id': currentUserId,
        'excluded_users': excludedUsers,
        'page_offset': 0,
        'page_limit': 3, // REDUCE from 5 to 3 for faster response
      }).timeout(const Duration(seconds: 3), onTimeout: () {
        return [];
      });
  ...
  Future<void> _loadData({bool loadMore = false}) async {
    ...
    try {
      ...
      // CREATE EXCLUDED USERS BASED ON TAB
      ...
      if (_selectedTab == 0) {
        ...
        final responseRaw = await _supabase.rpc('get_for_you_feed', params: {
          'current_user_id': currentUserId,
          'excluded_users':
              forYouExcludedUsers, // Use the extended exclusion list
  ...
  void _switchTab(int index) {
    ...
    if (index == 1) {
      ...
      // Following tab (index 0)
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        // LOAD FOLLOWING IDs ONLY WHEN SWITCHING TO FOLLOWING TAB
        if (_followingIds.isEmpty) {
