...
class _FeedScreenState extends State<FeedScreen> {
  ...
  // 🚀 FIX: Initialize controllers immediately, not deferred
  late final PageController _followingPageController;
  ...
  final Map<String, DateTime> _postFetchTimes = {};
  static const Duration _minFetchInterval = Duration(minutes: 10);

  // 🚀 FIX: Add initialization status tracking
  ...
  Future<void> _recordPendingViews() async {
    if (_pendingViews.isEmpty || _isDisposed || !mounted) {
  ...
  void initState() {
    ...
    // 🚀 FIX: Initialize controllers immediately in initState
    ...
    // 🚀 INSTANT: Show cached content immediately
    _loadCachedContentInstantly();

    // 🚀 BACKGROUND: Load fresh data later
  ...
  // 🚀 NEW: Load cached content without any delay
  Future<void> _loadCachedContentInstantly() async {
    if (_isDisposed) return;
    ...
    try {
      ...
      // 🚀 BACKGROUND: Load fresh data after UI is visible
      if (!_isDisposed && mounted) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!_isDisposed && mounted) {
  ...
  // 🚀 NEW: Prevent too frequent API calls
  bool _shouldFetchFreshData() {
    final now = DateTime.now();
    final lastFetch = _postFetchTimes[currentUserId] ?? DateTime(0);
    return now.difference(lastFetch) > _minFetchInterval;
  }

  // 🚀 UPDATED: Smarter background data loading
  Future<void> _loadFreshDataInBackground() async {
    if (!_shouldFetchFreshData() || _isDisposed) return;
    ...
    try {
      ...
      // Only fetch fresh posts if we don't have enough cached content
      if (_forYouPosts.length < 3 && !_isDisposed) {
        await _loadData();
      }

      if (!_isDisposed) {
        _postFetchTimes[currentUserId] = DateTime.now();
      }
    } catch (e) {
  ...
  // 🚀 UPDATED: Much faster bulk user fetch
  ...
  void _updatePostVisibility(
    ...
    if (!mounted || posts.isEmpty || _isDisposed) return;

    final previouslyPlayingPostId = _currentPlayingPostId;

    // 🚀 FIX: Use WidgetsBinding to schedule state update
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _startGuidelinesTimer() {
    _guidelinesTimer = Timer.periodic(const Duration(seconds: 5), (timer) {
      if (!_isPopupShown && !_isDisposed && mounted) {
  ...
  void _showGuidelinesPopup() {
    ...
    showDialog(
      ...
      if (mounted && !_isDisposed) setState(() => _isPopupShown = false);
    });
  }
  ...
  Future<void> _loadData({bool loadMore = false}) async {
    ...
    try {
      ...
      if (mounted && !_isDisposed) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted && !loadMore && !_isDisposed) {
            final currentPage =
      ...
      if (mounted && !_isDisposed) setState(() => _isLoadingMore = false);
    }
  }

  // 🎯 Switch tabs - only use cache for For You tab
  void _switchTab(int index) {
    if (_selectedTab == index || _isDisposed) return;
    ...
    if (index == 1) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        ...
        if (cachedPosts != null &&
          ...
          if (mounted && cachedPosts.isNotEmpty && !_isDisposed) {
            _updatePostVisibility(0, cachedPosts, true);
          }

          // 🚀 NEW: Load fresh data in background for this tab too
          await Future.wait([
            _loadBlockedUsers(),
            _loadFollowingIds(),
          ]);
          ...
          // No cached posts - load fresh data immediately
          await Future.wait([
            _loadBlockedUsers(),
            _loadFollowingIds(),
          ]);
  ...
  // 🚀 NEW: Optimized build method
  @override
  Widget build(BuildContext context) {
    final themeProvider = Provider.of<ThemeProvider>(context);
    final colors = _getColors(themeProvider);

    // 🚀 FIX: Show loading until fully initialized
    if (!_isInitialized) {
      return Scaffold(
    ...
    return Scaffold(
      backgroundColor: colors.backgroundColor,
      body: _buildOptimizedFeed(colors),
    );
  }

  // 🚀 NEW: Separate method for better performance
  Widget _buildOptimizedFeed(_ColorSet colors) {
    return Stack(
      children: [
        _selectedTab == 1
  ...
  // 🆕 RE-ADDED: Combined overlay content with tabs and message button
  ...
  Widget _buildPostsPageView(
    ...
    // 🆕 RE-ADDED: Scroll detection for hiding/showing overlay
