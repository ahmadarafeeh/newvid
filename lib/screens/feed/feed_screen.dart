...
class _FeedScreenState extends State<FeedScreen> {
  ...
  // 🎯 UPDATED: Cache keys for UNSEEN For You posts
  static const String _forYouCacheKey = 'cached_for_you_posts';
  static const String _forYouCacheTimestampKey = 'cached_for_you_timestamp';
  static const String _seenPostsKey = 'seen_posts';
  static const Duration _cacheValidityDuration =
      Duration(hours: 4); // Shorter cache for freshness
  ...
  void _pauseCurrentVideo() {
    // 🎯 UPDATED: Use VideoManager to pause all videos
  ...
  // 🎯 UPDATED: Track seen posts to avoid showing them again
// 🎯 UPDATED: Track seen posts to avoid showing them again
...
// 🎯 UPDATED: Mark post as seen
  Future<void> _markPostAsSeen(String postId) async {
    try {
      ...
      await prefs.setStringList('$_seenPostsKey$currentUserId',
          trimmedSeenPosts); // FIXED: Added underscore
    } catch (e) {}
  }

  // 🎯 UPDATED: Cache only UNSEEN For You posts
  Future<void> _cacheForYouPosts(List<Map<String, dynamic>> posts) async {
    try {
      ...
      if (unseenPosts.length >= 2) {
        ...
        await prefs.setString(_forYouCacheKey, jsonEncode(cacheData));

        // Mark these cached posts as seen so we don't cache them again
  ...
  // 🎯 UPDATED: Load cached UNSEEN posts
  Future<List<Map<String, dynamic>>?> _loadCachedForYouPosts() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final cachedData = prefs.getString(_forYouCacheKey);
      ...
      if (cachedData != null) {
        ...
        // Check if cache is valid (same user and within time limit)
  ...
  // 🎯 UPDATED: Clear cache (for debugging or when needed)
  Future<void> _clearForYouCache() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.remove(_forYouCacheKey);
    } catch (e) {}
  }
    ...
    setState(() {
      ...
      if (page < posts.length) {
        ...
        if (postId.isNotEmpty) {
          ...
          // 🎯 NEW: Mark post as seen when it becomes visible
  ...
  // 🎯 UPDATED: Load cached UNSEEN posts immediately
  ...
  // 🎯 UPDATED: Cache only UNSEEN posts when loading new data
  Future<void> _loadData({bool loadMore = false}) async {
    ...
    try {
      ...
      if (_selectedTab == 0) {
        ...
        // 🎯 CACHE THE FIRST 2 UNSEEN POSTS FOR INSTANT LOADING NEXT TIME
      ...
      if (mounted) {
        setState(() {
          if (_selectedTab == 0) {
            ...
            // 🎯 For You feed: Only replace if we have fresh data
            // This preserves cached UNSEEN posts until fresh data arrives
            if (loadMore) {
  ...
  Widget _buildForYouFeed(_ColorSet colors) {
    // 🎯 SHOW CACHED UNSEEN POSTS IMMEDIATELY - no loading state
    return _buildPostsPageView(
  ...
  Widget _buildPostsPageView(
    ...
    // 🎯 Show cached UNSEEN posts immediately, skeleton only if completely empty
    if (posts.isEmpty) {
  ...
  // 🎯 UPDATED: Navigation to messages with video pausing
  void _navigateToMessages() {
    // 🎯 ADDED: Pause all videos before navigating
    VideoManager.pauseAllVideos();
    ...
    // 🎯 ADDED: Small delay to ensure video is paused
