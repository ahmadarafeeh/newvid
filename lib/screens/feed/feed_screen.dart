...
class _FeedScreenState extends State<FeedScreen> with WidgetsBindingObserver {
  ...
  // Delayed cache update to ensure we have enough data
  Future<void> _delayedCacheUpdate() async {
    ...
    _delayedCacheUpdateTimer = Timer(Duration(seconds: 2), () async {
      ...
      if (_nextForYouBatch.isNotEmpty) {
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      // If we loaded cache, wait a bit before loading fresh data
      if (_cacheLoaded) {
        await Future.delayed(Duration(milliseconds: 800));
      }

      // Only try to load cache from FeedCacheService if we haven't attempted it yet
      if (!_cacheLoadAttempted) {
        final cachedPosts =
            await FeedCacheService.loadCachedForYouPosts(currentUserId);
      ...
      // 1. Cache was attempted but empty/null
      // 2. Cache was loaded in _loadCachedPostsLightningFast
      // 3. No cache available

      if (_essentialUiReady && _forYouPosts.isNotEmpty) {
        // We have cached posts from _loadCachedPostsLightningFast
        _nextForYouBatch = await _loadNextForYouBatch();
        _nextBatchLoaded = true;
      ...
      // No cache and no essential UI ready
      if (currentUserId.isEmpty) {
        _blockedUsers = [];
        _followingIds = [];
        await _loadData();
