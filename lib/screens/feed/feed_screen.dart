...
class _FeedScreenState extends State<FeedScreen> {
  ...
  // 🚀 NEW: Load cached posts without waiting for other data
  Future<void> _loadCachedPostsInstantly() async {
    ...
    try {
      ...
      if (cachedPosts != null && cachedPosts.isNotEmpty && mounted) {
        ...
        _bulkFetchUsers(cachedPosts).then((_) {
        ...
        setState(() {
          _cachedPostsLoaded = true; // Mark as loaded even if no cache
        });
      }
    } catch (e) {
      setState(() {
        _cachedPostsLoaded = true; // Continue even if cache fails
      });
    }
  }

  // 🚀 NEW: Load essential data that doesn't block cached posts
  Future<void> _loadEssentialDataInBackground() async {
    ...
    try {
      ...
      // 🚀 Only load fresh data if we don't have cached posts
  ...
  void initState() {
    ...
    // 🚀 STEP 1: Load cached posts IMMEDIATELY for instant display
    _loadCachedPostsInstantly();

    // 🚀 STEP 2: Load essential data in background
  ...
  void _onPageChanged(int page, bool isForYou) {
    if (isForYou) {
      ...
      // 🚀 NEW: Only replace with fresh data when user scrolls past cached posts
      if (_hasFreshData &&
  ...
  // 🚀 NEW: Replace cached posts with fresh data when user scrolls past them
  ...
  // 🚀 NEW: Load fresh data but don't replace cached posts until user scrolls
  ...
  void _switchTab(int index) {
    ...
    // 🚀 NEW: Reset scroll tracking when switching tabs
    ...
    if (index == 1) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        // 🚀 NEW: Use enhanced cache loading for For You tab
        if (!_cachedPostsLoaded || _forYouPosts.isEmpty) {
          final cachedPosts =
              await FeedCacheService.loadCachedForYouPosts(currentUserId);
          if (cachedPosts != null && cachedPosts.isNotEmpty && mounted) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // 🆕 RE-ADDED: Overlay with animated opacity for hide/show functionality
  ...
  // 🚀 OPTIMIZED: Build feed body with priority on cached content
  Widget _buildFeedBody(_ColorSet colors) {
    // Show cached posts immediately, even if other data is still loading
    if (_selectedTab == 1 && _forYouPosts.isNotEmpty) {
      return _buildPostsPageView(
          _forYouPosts, _forYouPageController, colors, true);
    }

    // Show skeleton only if we have no cached content
  ...
  // 🚀 NEW: Minimal skeleton for instant feedback
  ...
  // 🚀 OPTIMIZED: Improved skeleton feed that shows immediately
