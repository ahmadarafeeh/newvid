  ...
  Widget _buildVideoTrimmer() {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Show Save button only when not loading and not trimming
          if (!_isLoading && !_progressVisibility)
            TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        actions: [
          // Show Save button only when there's new media AND not loading
          if ((_image != null || _videoFile != null) && !_isLoading)
            _buildSaveButton(_isLoading, _saveProfile, colors),
        ],
      ),
      body: _isLoading
          ? Center(
