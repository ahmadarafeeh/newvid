...
// Helper class to check if URL is a video
class VideoUtils {
...
class _CommentCardState extends State<CommentCard> {
  ...
  // FORCED DARK MODE COLORS - No more theme provider dependency
  Color _getTextColor() {
    return const Color(0xFFd9d9d9); // Always dark mode text color
  }

  Color _getCardColor() {
    if (widget.forcedTransparent) {
      return Colors.transparent;
    }
    return const Color(0xFF121212); // Always dark mode card color
  }

  Color _getTransparentCardColor() {
    return Colors.black.withOpacity(0.15); // Always dark mode transparent
  }

  Color _getIconColor() {
    return const Color(0xFFd9d9d9); // Always dark mode icon color
  }

  // Build user avatar with video support
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: widget.forcedTransparent
              ? Border.all(
                  color:
                      Colors.white.withOpacity(0.05), // Even more subtle border
                  width: 0.3,
                )
              : null,
        ),
              ...
              children: [
                FutureBuilder<Map<String, dynamic>?>(
                  ...
                  builder: (ctx, userSnap) {
                    ...
                    return _buildUserAvatar(
                        photoUrl, 12, widget.forcedTransparent);
                  },
                ),
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      // UPDATED: Using imported VerifiedUsernameWidget
                      GestureDetector(
                        onTap: () {
                          Navigator.push(
                            context,
                  ...
                  children: [
                    IconButton(
                      ...
                      onPressed: () async {
                        ...
                        final bool previousLikeState =
                            _replyLikes[replyId] ?? false;
                        final int previousLikeCount =
                            _replyLikeCounts[replyId] ?? 0;

                        // Immediate UI update
                        setState(() {
                          _replyLikes[replyId] = !previousLikeState;
                          _replyLikeCounts[replyId] =
                              previousLikeCount + (previousLikeState ? -1 : 1);
                        });
                        ...
                        try {
                          ...
                          if (result['action'] == 'error') {
                            ...
                            setState(() {
                              _replyLikes[replyId] = previousLikeState;
                              _replyLikeCounts[replyId] = previousLikeCount;
                            });
                          ...
                          // If successful, the state is already correct from the optimistic update
                        } catch (e) {
                          // Revert on exception
                          setState(() {
                            _replyLikes[replyId] = previousLikeState;
                ...
                if (!widget.isReplying) ...[
                  PopupMenuButton<String>(
                    ...
                    color: widget.forcedTransparent
                        ? Colors.black.withOpacity(0.6) // More transparent
                        : cardColor,
                      ...
                      IconButton(
                        ...
                        onPressed: () async {
                          ...
                          try {
                            ...
                            // REVERT ON ERROR: If the API call fails, revert the optimistic update
                            setState(() {
