...
class SupabasePostsMethods {
  ...
  // ----------------------
  // COMPATIBILITY WRAPPERS (if you need them temporarily)
  // ----------------------

  // For compatibility with old code that expects the old signature
  // Only use this if you can't update all calling code immediately
  Future<String> uploadImageToStorageCompat(
      String childName, Uint8List file, bool isPost,
      {String contentType = 'image/jpeg'}) async {
    try {
      // Just use Supabase, ignore childName and isPost parameters
      String fileName = 'image_${DateTime.now().millisecondsSinceEpoch}.jpg';
      return await _storageMethods.uploadImageToSupabase(
        file,
        fileName,
        useUserFolder: true,
      );
  ...
  // For compatibility with old video upload code
  Future<String> uploadVideoToSupabaseSimpleCompat(
      String bucketName, Uint8List file, String fileName) async {
    try {
      // Ignore bucketName parameter (it's always 'videos' in new method)
      return await _storageMethods.uploadVideoToSupabase(
        file,
        fileName,
        useUserFolder: true,
      );
  ...
  // [REST OF YOUR EXISTING CODE REMAINS THE SAME - JUST UPDATE THE deletePost method above]
  // ... rest of your existing methods (likePost, comment, rate, etc.)
