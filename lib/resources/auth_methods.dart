...
class AuthMethods {
  ...
  // =============================================
  // 🔥 NEW: SUPABASE-ONLY METHODS (No Firebase)
  // =============================================

  // ----------------------
  // Supabase Google Sign-up/Sign-in
  // ----------------------
  ...
  // ----------------------
  Future<String> signUpWithEmailAndPasswordSupabase({
    required String email,
    required String password,
  }) async {
    try {
      print('🚀 Starting Supabase email/password sign-up for: $email');
      ...
      final AuthResponse response = await _supabase.auth.signUp(
        email: email,
        password: password,
        emailRedirectTo: 'ratedly://login-callback',
      );
      ...
      try {
        ...
        // Check if session exists (email might need verification)
  ...
  // ----------------------
  Future<String> loginWithEmailAndPasswordSupabase({
    required String email,
    required String password,
  }) async {
    try {
      ...
      print('✅ Supabase login successful: ${response.user!.id}');
      ...
      if (e.message?.contains('Invalid login credentials') ?? false) {
        return "Incorrect email or password";
      } else if (e.message?.contains('Email not confirmed') ?? false) {
        return "Please verify your email address first";
      } else if (e.message?.contains('User not found') ?? false) {
    ...
    try {
      ...
      if (userRecords.isEmpty) {
        ...
        try {
          ...
          print('✅ Created user record for new Supabase user');
        } catch (e) {
          print('❌ Error creating user record: $e');
        }
      ...
      print('❌ Error checking Supabase onboarding: $e');
      return "onboarding_required";
    }
  }
  ...
  Future<AppUser?> getSupabaseUserDetails() async {
    try {
      ...
      print('❌ Error getting Supabase user details: $e');
      return null;
    }
  }

  // =============================================
  // 🔄 EXISTING FIREBASE METHODS (For Migration)
  // =============================================
