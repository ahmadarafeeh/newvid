...
class SupabaseProfileMethods {
  ...
  // NEW: Method to delete profile media (both image and video)
  Future<void> deleteProfileMedia(String mediaUrl) async {
    try {
      if (mediaUrl.isEmpty || mediaUrl == 'default') {
        return;
      }
      ...
      if (_isVideoUrl(mediaUrl)) {
        ...
        // It's an image from Supabase - delete it
        await StorageMethods().deleteImage(mediaUrl);
      }
      // Note: Firebase photos and Google photos are not deleted (as per original logic)
    } catch (e) {
      // Error deleting profile media
    }
  }
  ...
  Future<void> _recordPushNotification({
    ...
    try {
      ...
      // Error recording push notification
    }
  }
...
// Get profile viewers with user info
  ...
  Future<void> _deleteUserActorNotifications(String uid) async {
    try {
      ...
      // Error deleting actor notifications
    }
  }
  ...
  Future<void> _deleteUserPostViews(String uid) async {
    try {
      ...
      // Error deleting user post views
    }
  }
  ...
  Future<String> deleteEntireUserAccount(
    ...
    try {
      ...
      // 5. Clean up followers/following relationships
      // Get followers
      ...
      // 6. Delete user's comments, ratings, messages, etc.
      ...
      // 9. Delete user document from users table
      await _supabase.from('users').delete().eq('uid', uid);

      // 10. Delete profile media from storage
      ...
      // 11. Delete Firebase user account if applicable
      if (isFirebaseUser && firebaseUser != null) {
        await firebaseUser.delete();
      }

      // 12. For Supabase users, sign out from Supabase (auth record remains but user record is gone)
      if (isSupabaseUser) {
